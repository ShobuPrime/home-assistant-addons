...
echo -e "${BLUE}=== Arcane Addon Builder ===${NC}"
...
case $ARCH in
    ...
    *)
        ...
        echo -e "${YELLOW}Arcane only supports amd64 and aarch64${NC}"
        exit 1
        ;;
esac
...
# Get Arcane version from build.yaml
ARCANE_VERSION=$(grep "ARCANE_VERSION:" build.yaml | cut -d':' -f2 | tr -d ' ')
echo -e "Arcane version: ${YELLOW}${ARCANE_VERSION}${NC}"
...
docker build \
    ...
    --build-arg BUILD_DESCRIPTION="Arcane Docker Manager for Home Assistant" \
    ...
    --build-arg ARCANE_VERSION="${ARCANE_VERSION}" \
    -t "${FULL_IMAGE}" \
    .
...
if [ $? -eq 0 ]; then
    ...
    echo -e "  ${YELLOW}docker run --rm -it -p 3552:3552 -p 3553:3553 -v /var/run/docker.sock:/var/run/docker.sock ${FULL_IMAGE}${NC}"
    echo ""
